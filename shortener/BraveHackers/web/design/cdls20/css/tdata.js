...
(function($) {	
	...
	$.fn.getRecommendations = function(args, callBackfunction) {
		var domain = 'domain' in args ? args.domain : 'tdata-recommendationsdev.stage.att.net',
			...
			serviceURL= 'serviceURL' in args ? args.serviceURL : 'https://' + domain + '/nt/TDiceRecommendations/' + versionMajor + '/' + versionMinor + '/recommendations/http/' + consumptionEngine + '/PG'+ pageID+ '?deviceType=' + deviceType+'&userId='+userID+'&userIdType=Email&callback=?'
...
/*
	...
	function executeDataTdata(data, callBackfunction) {
					...
					for (j = 0; j < tdataArr.length; j++) {
									...
									if (!found)	$().replaceContent({statusCode: '404'}, tdataObject, replaceSpace);
					}
...
//		console.log('lineCount:' + lineCount);
		...
		if (contentWhiteSpace<0) contentWhiteSpace=0;
