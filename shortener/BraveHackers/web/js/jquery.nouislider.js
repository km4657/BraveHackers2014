		...
		methods = {
			create : function () {
				...
				return this.each(function () {
					...
					// handles
					handlehtml = '<div class="sliderbar" role="slider" aria-labelledby="slider-label" aria-valuemin="0" aria-valuemax="100" aria-valuenow=""><button><i class="invisible"></i></button></div>',
					...
					for (var i = 0; i < settings.handles; i++) {
						...
						handles[i].find('button').on(onEvent, function (e) {
					...
					for (var i = 0; i < handles.length; i++) {
						...
						if (typeof settings.serialization.to[i] == 'string') {
							handles[i].data('input',
								slider.append('<input type="hidden" id="sliderValNow" name="' + settings.serialization.to[i] + '">').find('input:last')
								.val(val)
								.change(function (a) {									
									a.stopPropagation();
								})
